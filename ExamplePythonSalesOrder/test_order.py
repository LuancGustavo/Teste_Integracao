...
def test_new_processar_pedido():
    cliente = Cliente(1, "alambida")
    cliente_repository = ClienteRepository()
    cliente_repository.adicionar_cliente(cliente)

    product1 = Produto(1, "cebola", 50, 100)
    produto_repository = ProdutoRepository()
    produto_repository.adicionar_produto(product1)
...
def test_new_baixar_estoque():
    cliente = Cliente(1, "luanzin")
    cliente_repository = ClienteRepository()
    cliente_repository.adicionar_cliente(cliente)

    product1 = Produto(1, "tomate", 50, 100)
    produto_repository = ProdutoRepository()
    produto_repository.adicionar_produto(product1)
    ...
    product1.baixar_estoque(40)

    assert product1.estoque == 45

def test_baixar_estoque():
    product1 = Produto(1, "tangerina", 50, 100)
    num = 85

    product1.baixar_estoque(num)

    assert product1.estoque == 15
